...
- GUI exits or crashes: the disposable attachment closes, and tmux keeps work. The desktop-side flush path distinguishes `desktop-stop-runtime`, `desktop-window-closed` and `desktop-killed` for postmortem clarity; a window close or GUI crash no longer signals the runtime child (M1.6). The OS lock and the runtime survive across desktop restarts; the next launch re-attaches to the live runtime via `tryAttachRuntime` or spawns a fresh child.
- Explicit "Stop runtime": the desktop forwards SIGTERM through `RuntimeHandle.stop`, drains accepted desktop IPC and acknowledged drafts under the watchdog budget, and exits. tmux work ends only along this path.
