...
The next concrete work is **M1.5 (bounded terminal-owned paste queue) and M1.6 (separate close-window from stop-runtime)**. The M1.3+M1.4 independent-runtime milestones are now shipped: the desktop spawns the OS-locked `dist/runtime/index.cjs` through `helpers/runtime_lock.py`, the deployment test covers duplicate-owner BUSY, shared-socket-directory refusal, SIGKILL lock-inode preservation and wrong-token handshake failure, and the in-process workspace fallback is gone. M2 transactional storage and the M3 managed-repair pilot remain the next sequence beyond M1.5/M1.6.
